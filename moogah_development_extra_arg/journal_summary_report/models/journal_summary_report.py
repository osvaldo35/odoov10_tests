...
class JournalSummaryReport(models.AbstractModel):
    ...
    def _lines(self, line_id=None):
        ...
        if summary_section:
            move_line_ids=[]
            for section in summary_section:
                for move in section['move_lines']:
                    move_line_ids.append(move.id)
            account_moves = account_moves.filtered(
                        lambda m: m.id not in move_line_ids)
    ...
    def _xlsx_lines(self, line_id=None):
        ...
        if summary_section:
            ...
            for section in summary_section:
                for move in section['move_lines']:
                    move_line_ids.append(move.id)
            account_moves = account_moves.filtered(lambda m: m.id not in move_line_ids)
        if ctx.get('sort_by') in ['date', 'monthly']:
            sorted_account_moves = account_moves.sorted(key=lambda r: r.date)
        else:
            sorted_account_moves = account_moves.sorted(key=lambda r: r.name)
    ...
    def get_account_invoices_move_ids(self, start_date, end_date, type):
        account_invoices = self.env['account.invoice'].search([('journal_id.type', '=', type),
                                                              ('move_id', '!=', False),
                                                               ('move_id.document_type_id.internal_type', 'in',
                                                                ['invoice', 'debit_note', 'credit_note', 'ticket'])])
        if account_invoices:
            return account_invoices.mapped('move_id').filtered(
                lambda m: m.date >= start_date and m.date <= end_date)
        return []


    def get_account_payment_group_move_ids(self, start_date, end_date, internal_types):
        customer_receipts_move_ids = self.env['account.payment.group'].search(
            [('payment_ids.journal_id.type', 'in', ['bank', 'cash']),
             ('payment_ids.move_line_ids.move_id.document_type_id.internal_type', 'in',
              internal_types)])
        if customer_receipts_move_ids:
            return customer_receipts_move_ids.mapped('payment_ids').mapped('move_line_ids').filtered(
                lambda m: m.move_id.date >= start_date and m.move_id.date <= end_date)
        return []
    ...
    def get_summary_section_move_line(self, type, summary_section, lines, sort_by, number, move=None, last_move=None):
        ...
        for summary in summary_section_filter:
            ...
            move_line = self.env['account.move.line'].search([
                ('move_id', 'in', summary['move_lines'].ids)])
            accounts = move_line.mapped('account_id')
            for account in accounts:
                filter_move_line = move_line.filtered(lambda l: l.account_id.id == account.id)
                if type == 'html':
