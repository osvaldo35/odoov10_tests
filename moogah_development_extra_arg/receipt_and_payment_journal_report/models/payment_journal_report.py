...
class PaymentJournalReport(models.AbstractModel):
    ...
    def _xlsx_lines(self, line_id=None):
            ...
            for payment_group in sorted_apg:
                ...
                if context.get('journal_ids'):
                    journals = payment_group.payment_ids.filtered(
                        lambda x: x.journal_id in context.get('journal_ids')).mapped('journal_id')
                if len(journals.ids) == 1:
                    payment_amount = sum(payment_group.payment_ids.filtered(
                        lambda x: x.journal_id == journals).mapped('amount'))
                    currency_amount = payment_group.company_id.currency_id.compute(payment_amount, used_currency)
                    ...
                    for move_line in move_lines:
                        ...
                        lines.append({
                            ...
                            'total_journal': not first_line and total_amount or False,
                            'move_id': False,
                            'name': 'Line',
                            'colspan': 0,
                            'footnotes': {},
                            'columns': [supplier_name, number, payment_date,
                                        ...
                                        not first_line and
                                        str(self._format(payment_amount, payment_group.currency_id)) or '',
                                        not first_line and
                                        str(self._format(total_amount, used_currency)) or ''],
                            'level': 1,
                        })
                        first_line = True
                    if not move_lines or payment_group.state == 'draft':
                        ...
                        lines.append({
                            ...
                            'columns': [supplier.name, payment_group.display_name, payment_date,
                                        '', str(self._format(payment_amount, payment_group.currency_id)),
                                        str(self._format(total_amount, used_currency))],
                            'level': 1,
                        })
                        first_line = True
                    ...
                    last_journal = False
                    payment_amount_journal = journal_array[0]['amount']
                    i = 0
                    for move_line in move_lines:
                        number = not first_line and payment_group.display_name or ''
                        payment_date = not first_line and datetime.strptime(payment_group.payment_date,
                        ...
                        supplier_name = not first_line and supplier.name or ''
                        if not last_journal:
                            payment_amount_journal = journal_array[i]['amount']
                            currency_amount_journal = payment_group.company_id.currency_id.compute(
                        ...
                        lines.append({
                            'id': move_line.id,
                            'type': 'line_id',
                            'journal_id': not last_journal and journal_array[i]['journal_id'] or False,
                            'total_journal': not last_journal and journal_array[i]['amount'] or False,
                            'move_id': False,
                            'name': 'Line',
                            'colspan': 0,
                            'footnotes': {},
                            'columns': [supplier_name, number, payment_date,
                                        move_line.invoice_id.display_name,
                                        not last_journal and
                                        journal_array[i]['code'] + ' ' + str(
                                            self._format(payment_amount_journal, payment_group.currency_id)) or '',
                                        not last_journal and
                                        str(self._format(total_amount, used_currency)) or ''],
                            'level': 1,
                        })
                    ...
                    for journal in filter(lambda x: not x.get('show'), journal_array):
                        ...
                        currency_amount_journal = move_line.company_id.currency_id.compute(
                            payment_amount_journal, used_currency)
                        total_amount += currency_amount_journal
                        lines.append({
                            ...
                            'total_journal': journal['amount'],
                            'move_id': False,
                            'colspan': 0,
                            'name': 'Line',
                            'footnotes': {},
                            'columns': [supplier_name, number, payment_date, '', journal_array[i]['code'] + ' ' +
                                        str(self._format(payment_amount_journal, payment_group.currency_id)),
                                        not first_line and str(self._format(total_amount, used_currency)) or ''],
                            'level': 1,
                        })
                        if not move_lines or payment_group.state == 'draft':
                            first_line = True
        ...
        if total_payment_journal_amount:
            lines.append({
                ...
                'footnotes': {},
                'columns': ['', '', None, '', '',
                            str(self._format(total_payment_journal_amount, used_currency))],
                'level': 1,
            })
            lines.append({
                'id': 0,
        ...
        if payment_quantity:
            lines.append({
                ...
                'colspan': 0,
                'footnotes': {},
                'columns': ['', '', None, '', '', str(payment_quantity)],
                'level': 1,
            })
        return lines
    ...
    def _overview_xlsx_lines(self, line_id=None):
            ...
            if len(journals.ids) == 1:
                ...
                for journal in filter(lambda x: not x.get('show'), journal_array):
                    ...
                    lines.append({
                        ...
                        'footnotes': {},
                        'columns': [number, payment_date, supplier_name, '',
                                    journal_array[i]['code'] + ' ' + str(self._format(
                                        payment_amount_journal, payment_group.currency_id))],
                        'level': 1,
                    })
                    if not move_lines or payment_group.state == 'draft':
                        first_line = True
        ...
        if first_line:
            lines.append({
                ...
                'footnotes': {},
                'columns': ['', None, '', '',
                            str(self._format(total_payment_amount, used_currency))],
                'level': 1,
            })
            lines.append({
                'id': 0,
                    ...
                    if len(journals.ids) == 1:
                        ...
                        if first_line:
                            lines.append({
                                ...
                                'footnotes': self.env.context['context_id']._get_footnotes('line_id',
                                                                                           payment_group.id),
                                'columns': ['', '', None, '', (str(self._format(total_amount, used_currency)),
                        ...
                        for journal in filter(lambda x: not x.get('show'), journal_array):
                            ...
                            lines.append({
                                ...
                                'name': '',
                                'footnotes': {},
                                'columns': ['', number, payment_date, '', journal_array[i]['code'] + ' ' +
                                            str(self._format(payment_amount_journal, payment_group.currency_id))],
                                'level': 2,
                            })
                            if not move_lines or payment_group.state == 'draft':
                ...
                if first_line and sorted_apg:
                    lines.append({
                        ...
                        'columns': [_('Total per Supplier’s Payments'), '', None, '',
                                    (str(self._format(total_supplier_amount, used_currency)),
                                     "", "border-top:1px solid")],
                        'level': 1,
                    })
                    lines.append({
                    ...
                    if first_line:
                        lines.append({
                            ...
                            'footnotes': self.env.context['context_id']._get_footnotes(
                                ...
                                payment_group.id),
                            'columns': ['', None, '', '',
                                        (str(self._format(total_amount, used_currency)), "", "border-top:1px solid")],
                            'level': 1,
                        })
                else:
                    journal_array = []
    ...
    def get_payment_journal_lines(self, lines):
        ...
        for journal_line in filter(lambda x: x.get('journal_id', False), lines):
            journal = filter(lambda x: x.get('journal_id', False)
                             ...
                             total_journal_lines)
            if journal:
                journal[0]['total_payment'] += journal_line['total_journal']
            else:
                total_journal_lines.append({'journal_id': journal_line['journal_id'],
                                            ...
                                            'total_payment': journal_line['total_journal']})
    ...
    def get_report_type(self):
        return self.env.ref('receipt_and_payment_journal_report.payment_journal_report_type')

    def get_template(self):
        return 'receipt_and_payment_journal_report.report_payment_journal'
...
class PaymentJournalContextReport(models.TransientModel):
    ...
    def get_html_and_data(self, given_context=None):
        ...
        if given_context:
            ...
            if given_context.get('active_id', False):
                wizard = self.env['payment.journal.report.wizard'].browse(given_context.get('active_id'))
                if wizard and self.wizard_id != wizard.id:
                    self.write({'date_from': wizard.start_date,
                                ...
                                'detail_level': wizard.detail_level,
                                'date_filter': 'custom',
                                'wizard_id': wizard.id,
                                })
    ...
    def get_per_supplier_columns_names(self):
        return [_("Pay. No"), _("Pay. Date"), _("Invoice No"), _("Pay. Amount")]

    def get_per_supplier_xlsx_columns_names(self):
    ...
    @api.multi
    def get_per_supplier_columns_types(self):
        return ["text", "text", "date", "text", "text"]

    @api.multi
    def get_per_supplier_xlsx_columns_types(self):
        return ["text", "text", "date", "text", "text", "text"]
    ...
    def get_xlsx(self, response):
        ...
        if self.detail_level == 'per_supplier':
            columns_names = self.with_context(is_xls=True).get_per_supplier_xlsx_columns_names()
        ...
        sheet.merge_range('A' + str(limit) + ':' + journal_letter_right + str(limit), _('Payment Journal'),
                          level_0_style_left)
        sheet.merge_range(total_letter_left + str(limit) + ':' + total_letter_right + str(limit), _('Total'),
                          level_0_style_right)
        ...
        for journal_line in journal_lines:
            sheet.merge_range('A' + str(limit) + ':' + journal_letter_right + str(limit), journal_line['journal_name'],
                              journal_style_left)
            sheet.merge_range(total_letter_left + str(limit) + ':' + total_letter_right + str(limit),
                              journal_line['total_payment'], journal_style_right)
            limit += 1
    ...
    def get_pdf(self):
        ...
        body = self.env['ir.ui.view'].render_template(
            "receipt_and_payment_journal_report.report_payment_journal_letter",
            values=dict(rcontext, lines=lines, journal_lines=journal_lines, footnotes=footnotes, report=report_obj,
                        context=self),
        )
    ...
    def get_supplier(self, partner_id):
        if partner_id:
    ...
    def get_status(self, draft, confirmed, posted):
        states = {}
        if draft:
