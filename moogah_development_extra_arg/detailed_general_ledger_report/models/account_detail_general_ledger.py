...
class ReportAccountDetailGeneralLedger(models.AbstractModel):
    ...
    def get_lines(self, context_id, line_id=None,  analytic_tag_id=None):
        if type(context_id) == int:
            context_id = self.env['account.context.detail.general.ledger'].search([['id', '=', context_id]])
    ...
    def _do_query(self, line_id, analytic_tag_id=False, group_by_account=True, limit=False):
        if self._context.get('analytic_account_dimension_id'):
            return self._do_query_tag(line_id, analytic_tag_id, group_by_account, limit)
        if group_by_account:
    ...
    def _do_query_tag(self, line_id, analytic_tag_id, group_by_account=True, limit=False):
        if group_by_account:
        ...
        if analytic_tag_id:
            ...
            tag_ids = self._context.get('analytic_account_dimension_id').analytic_tag_ids.ids
        for tag in tag_ids:
            analytic_tag_ids = self.env['account.analytic.account'].search([('tag_ids', 'in', [tag])])
            tag_clause = self._get_tag_clause(tag,analytic_tag_ids.ids)
            with_sql, with_params = self._get_with_statement(user_types)
            tables, where_clause, where_params = self.env['account.move.line']._query_get()
            line_clause = line_id and ' AND \"account_move_line\".account_id = ' + str(line_id) or ''
            line_clause += tag_clause
            query = sql % (select, tables + ',"account_analytic_tag"', where_clause, line_clause)
            self.env.cr.execute(with_sql + query, with_params + where_params)
    ...
    def _get_tag_clause(self, tag, analytic_ids):
        ...
        if analytic_ids and len(analytic_ids) == 1:
            ...
            tag_clause_1 = ' AND(( \"account_move_line\".analytic_account_id IN ' +str(tuple(analytic_ids,)) + ') '
        else:
        ...
        tag_clause_2 = ' OR(\"account_move_line\"."id" in (SELECT \"account_analytic_tag_account_move_line_rel\"."account_move_line_id" ' \
                      ...
                      ' WHERE \"account_analytic_tag_account_move_line_rel\"."account_analytic_tag_id" IN ( ' + str(tag) + ')' + '))) '
    ...
    def do_query(self, line_id):
        ...
        compute_table = {
            ...
            for a in self.env['account.account'].browse([k[0] for k in results if k])
        }
        results = dict([(
            k[0], {
    ...
    def do_query_tag(self,line_id, analytic_tag_id):
        results = self._do_query(line_id, analytic_tag_id, group_by_account=True, limit=False,)
        used_currency = self.env.user.company_id.currency_id
        compute_table = {
            a.id: a.company_id.currency_id.compute
            for a in self.env['account.account'].browse([k[0] for k in results if k])
        ...
        for k in results:
            if not results_tag.get(k[1]):
                results_tag[k[1]] = [{k[0]:{'balance':compute_table[k[0]](k[2], used_currency) if k[0] in compute_table else k[2],
                  'amount_currency': k[3],
                  'debit': compute_table[k[0]](k[4], used_currency) if k[0] in compute_table else k[4],
                  'credit': compute_table[k[0]](k[5], used_currency) if k[0] in compute_table else k[5],}}]
            else:
                results_tag[k[1]].append({k[0]:{'balance':compute_table[k[0]](k[2], used_currency) if k[0] in compute_table else k[2],
                  'amount_currency': k[3],
                  'debit': compute_table[k[0]](k[4], used_currency) if k[0] in compute_table else k[4],
                  'credit': compute_table[k[0]](k[5], used_currency) if k[0] in compute_table else k[5],}})
        return results_tag
    ...
    def group_by_account_id(self, line_id):
        ...
        if context.get('analytic_account_dimension_id'):
            base_domain += ['|', ('analytic_account_id.tag_ids', 'in', context['analytic_account_dimension_id'].analytic_tag_ids.ids), ('analytic_tag_ids', 'in', context['analytic_account_dimension_id'].analytic_tag_ids.ids)]
    ...
    def group_by_tag_id(self, line_id, analytic_tag_id):
        ...
        initial_bal_date_to = datetime.strptime(self.env.context['date_from_aml'], "%Y-%m-%d") + timedelta(days=-1)
        initial_bal_results = self.with_context(date_to=initial_bal_date_to.strftime('%Y-%m-%d')).do_query_tag(line_id,analytic_tag_id)
        unaffected_earnings_xml_ref = self.env.ref('account.data_unaffected_earnings')
        unaffected_earnings_line = True  # used to make sure that we add the unaffected earning initial balance only once
        if unaffected_earnings_xml_ref:
            # compute the benefit/loss of last year to add in the initial balance of the current year earnings account
            last_day_previous_fy = self.env.user.company_id.compute_fiscalyear_dates(
                datetime.strptime(self.env.context['date_from_aml'], "%Y-%m-%d"))['date_from'] + timedelta(days=-1)
            unaffected_earnings_results = self.with_context(date_to=last_day_previous_fy.strftime('%Y-%m-%d'),
        ...
        if context.get('account_tag_ids'):
            base_domain += [('account_id.tag_ids', 'in', context['account_tag_ids'].ids)]
        if context.get('analytic_tag_ids'):
            base_domain += ['|', ('analytic_account_id.tag_ids', 'in', context['analytic_tag_ids'].ids),
                            ('analytic_tag_ids', 'in', context['analytic_tag_ids'].ids)]
        if context.get('analytic_account_ids'):
            base_domain += [('analytic_account_id', 'in', context['analytic_account_ids'].ids)]
        if context.get('analytic_account_dimension_id'):
            base_domain += ['|', (
            'analytic_account_id.tag_ids', 'in', context['analytic_account_dimension_id'].analytic_tag_ids.ids),
                            ('analytic_tag_ids', 'in', context['analytic_account_dimension_id'].analytic_tag_ids.ids)]
        if context.get('account_ids'):
            base_domain += [('account_id', 'in', context['account_ids'].ids)]
        for tag_id, values in results.items():
            ...
            for data in values:
                for account_id, account_result in data.items():
                    ...
                    accounts[account]['initial_bal'] = {'balance': 0, 'amount_currency': 0, 'debit': 0,
                                                                    'credit': 0}
                    if tag_id in initial_bal_results:
                        account_initial_bal = filter(lambda x: account_id in x, initial_bal_results[tag_id])
                        if account_initial_bal:
                            accounts[account]['initial_bal'] = account_initial_bal[0].get(account_id,
                    ...
                    aml_ids = self.with_context(**aml_ctx)._do_query(account_id, tag_id, group_by_account=False)
                    aml_ids = [x[0] for x in aml_ids]
                    accounts[account]['lines'] = self.env['account.move.line'].browse(aml_ids)
                    # if the unaffected earnings account wasn't in the selection yet: add it manually
                    if not unaffected_earnings_line and unaffected_earnings_results['balance']:
    ...
    def _get_taxes(self):
        ...
        for tax in self.env['account.tax'].browse(ids):
            ...
            if self.env.context['context_id'].journal_ids and self.env.context['context_id'].journal_ids[0].type == 'sale':
    ...
    def _lines(self, line_id=None, analytic_tag_id=None):
        ...
        if not context['analytic_account_dimension_id']:
            ...
            grouped_accounts = self.with_context(date_from_aml=context['date_from'], date_from=context['date_from'] and
            ...
            for account in sorted_accounts:
                ...
                if context['balance'] == 'with_balance' and balance or context['balance'] == 'without_balance' and not balance or \
                ...
                if include and (account in context['context_id']['unfolded_accounts'] or unfold_all):
                    ...
                    domain_lines = [{
                        ...
                        'columns': ['', '', '', '', initial_currency, self._format(initial_debit),
                    ...
                    domain_lines.append({
                        ...
                        'footnotes': self.env.context['context_id']._get_footnotes('o_account_reports_domain_total',
                                                                                   account.id),
                        'columns': ['', '', '', '', amount_currency, self._format(debit), self._format(credit),
            ...
            if len(context['context_id'].journal_ids) == 1 and context['context_id'].journal_ids.type in ['sale',
                ...
                lines.append({
                    ...
                    'name': _('Total'),
                    'footnotes': {},
                    'columns': ['', '', '', '', '', self._format(total['debit']), self._format(total['credit']),
    ...
    def _tag_lines(self, line_id=None, analytic_tag_id=None):
        ...
        lines = []
        company_id = context.get('company_id') or self.env.user.company_id
        grouped_tag_accounts = self.with_context(date_from_aml=context['date_from'], date_from=context['date_from'] and
                                                                                           company_id.compute_fiscalyear_dates(
                                                                                               datetime.strptime(
                                                                                                   context[
                                                                                                       'date_from'],
            ...
            line_id, analytic_tag_id)  # Aml go back to the beginning of the user chosen range but the amount on the account line should go back to either the beginning of the fy or the beginning of times depending on the account
        sorted_tag_accounts = sorted(grouped_tag_accounts, key=lambda a: a.id)
        unfold_all = context.get('print_mode') and not context['context_id']['unfolded_accounts']
        for tag in sorted_tag_accounts:
            sorted_accounts = sorted(grouped_tag_accounts[tag], key=lambda a: a.code)
            ...
            for account in sorted_accounts:
                ...
                include = False
                amount_currency = '' if not account.currency_id else self._format(
                    grouped_tag_accounts[tag][account]['amount_currency'], currency=account.currency_id)
                ...
                if include and account in context['context_id']['unfolded_accounts'] or unfold_all:
                    initial_debit = grouped_tag_accounts[tag][account]['initial_bal']['debit']
                    initial_credit = grouped_tag_accounts[tag][account]['initial_bal']['credit']
                    initial_balance = grouped_tag_accounts[tag][account]['initial_bal']['balance']
                    initial_currency = '' if not account.currency_id else self._format(
                        grouped_tag_accounts[tag][account]['initial_bal']['amount_currency'], currency=account.currency_id)
                    domain_lines = []
                    domain_lines = [{
                        'id': account.id,
                        'tag_id': tag.id,
                    ...
                    progress = initial_balance
                    amls = amls_all = grouped_tag_accounts[tag][account]['lines']
                    too_many = False
                    if len(amls) > 80 and not context.get('print_mode'):
                        amls = amls[:80]
                        too_many = True
    ...
    def get_balace(self, tag, grouped_tag_accounts):
        return filter(lambda x: not(not x['balance']),grouped_tag_accounts[tag].values())
    ...
    def get_report_type(self):
        return self.env.ref('detailed_general_ledger_report.account_report_type_detail_general_ledger')
...
class account_context_general_ledger(models.TransientModel):
    ...
    unfolded_accounts = fields.Many2many('account.account', 'context_to_account_dgl', string='Unfolded lines')
    journal_ids = fields.Many2many('account.journal', relation='account_report_dgl_journals')
    available_journal_ids = fields.Many2many('account.journal', relation='account_report_dgl_available_journal', default=lambda s: [(6, 0, s.env['account.journal'].search([]).ids)])
    ...
    def get_columns_names(self):
        return [_("Date"), _("Document"),_("Communication"), _("Partner"), _("Currency"), _("Debit"), _("Credit"), _("Balance")]
