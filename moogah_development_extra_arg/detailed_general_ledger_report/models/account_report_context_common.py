...
class AccountReportAnalyticManager(models.TransientModel):
    ...
    analytic_account_dimension_id = fields.Many2one('account.analytic.dimension', string='Dimension')
    ...
    def get_available_analytic_account_dimension_id_and_names(self):
        ...
        if self.create_uid.id in group_analytic.users.ids:
            return [[t.id, t.name] for t in self.env['account.analytic.dimension'].search([])]
        return []
...
class AccountReportAccountManager(models.TransientModel):
    ...
    account_ids = fields.Many2many('account.account', relation='account_report_context_account_rel')
    account_type_ids = fields.Many2many('account.account.type', relation='account_report_context_account_type_rel')
    ...
    def get_available_account_type_ids_and_names(self):
        return [[a.id, a.name] for a in self.env['account.account.type'].search([])]

    @api.multi
    def get_available_account_tag_and_names(self):
        return [[a.id, a.name] for a in self.env['account.account.tag'].search([])]
...
class AccountReportContextCommon(models.TransientModel):
    ...
    account_manager_id = fields.Many2one('account.report.account.manager', string='Account Filters Manager',
                                         required=True, ondelete='cascade')
    balance = fields.Selection([('all_balance', 'All Balance'), ('with_balance', 'With Balance'),
                                ('without_balance', 'Without Balance')], string='Balance', default='all_balance')

    def _report_name_to_report_model(self):
        res = super(AccountReportContextCommon, self)._report_name_to_report_model()
        res.update({'detailed_general_ledger': 'account.detail.general.ledger'})
        return res


    def _report_model_to_report_context(self):
        res = super(AccountReportContextCommon, self)._report_model_to_report_context()
        res.update({'account.detail.general.ledger': 'account.context.detail.general.ledger'})
        return res
        ...
        if report_model == 'account.detail.general.ledger':
            ...
            for c in self.env[context_model].search(domain):
                if c.available_company_ids and \
                    c.available_company_ids <= self.env['account.report.multicompany.manager']._default_company_ids():
                    context = c
                    break
            ...
            return super(AccountReportContextCommon, self).return_context(report_model, given_context, report_id)
